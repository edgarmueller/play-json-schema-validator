      ...
      Rule.fromMapping { json =>
        (schema.constraints.any._if, schema.constraints.any._then, schema.constraints.any._else) match {
          case (Some(ifSchema), Some(thenSchema), Some(elseSchema)) =>
            if (ifSchema.validate(json, context).isSuccess && thenSchema.validate(json, context).isSuccess) {
              Success(json)
            } else if (ifSchema.validate(json, context).isFailure && elseSchema.validate(json, context).isSuccess) {
              Success(json)
            } else {
              failure(
                "else",
                Messages("if.then.else", json),
                context.schemaPath,
          ...
          case (Some(ifSchema), Some(thenSchema), None) =>
            if (ifSchema.validate(json, context).isSuccess && thenSchema.validate(json, context).isSuccess) {
              Success(json)
            } else if (ifSchema.validate(json, context).isFailure) {
              Success(json)
            } else {
              failure(
                "then",
