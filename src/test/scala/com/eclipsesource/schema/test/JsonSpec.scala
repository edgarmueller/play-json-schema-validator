...
trait JsonSpec extends FragmentBuilder {
  ...
  def validate(name: String, folder: String = "draft4")(implicit reads: Reads[SchemaType]): Fragments =
  ...
  def validateMultiple(names: (String, Seq[String])*)(implicit reads: Reads[SchemaType]): Fragments = {
    try {
  ...
  def validateFragments(name: String, folder: String)(implicit reads: Reads[SchemaType]): Fragments =
    s2"""|$name should be ok $p
         |${examplesFromUrl(getClass.getResource(s"/$folder/$name.json"))}""".stripMargin

  def examplesFromUrl(url: URL)(implicit reads: Reads[SchemaType]): Fragments = {
  ...
  def fromUrl(url: URL)(implicit reads: Reads[SchemaType]): Either[String, Seq[(String, Seq[SpecResult])]] = {
  ...
  private def executeSpecs(jsonSpecs: Seq[JsValue])(implicit reads: Reads[SchemaType]): Seq[(String, Seq[SpecResult])] = {
